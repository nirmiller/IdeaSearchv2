...
def get_google_query(search):
    google_query_results = []
    page_depth = 3

    for i in range(1, page_depth):
        try:
            page = i
            start = (page - 1) * 10 + 1
            ...
            for i, search_item in enumerate(search_items, start=1):
                ...
                google_query_results.append(query(title, long_description, ['GOOGLE_WEBSITE', search_item['link']]))
        except Exception as e:
            print("Error with google api", e)
        print('Google Search Length', len(google_query_results))
        return google_query_results

    return google_query_results


def get_patent_query(search):
    ...
    try:
        ...
        for d in data['patents']:
            patent_query_results.append(query(d['patent_title'], d['patent_abstract'], ['PATENT', d['patent_number'], d['patent_date']]))
    except Exception as e:
...
def searcher(search_query):
  ...
  try:
    if len(search_query.title) < 70:
      search_web = get_google_query(search_query) + get_scholar_query(search_query) + get_youtube_query(search_query)
  except Exception as e:
      print(e)
